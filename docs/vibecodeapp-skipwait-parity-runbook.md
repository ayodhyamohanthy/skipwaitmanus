...
| **Referrer love** | An employee helps safely. | Server-derived private impact history with aggregate reviews, approvals, and participant-recorded forward milestones; optional availability preferences; and acknowledgment of completed helpful actions. | Repeat voluntary contribution. | No public leaderboard, employee ranking, candidate record, outcome reward, or referral quota. |
...
| Referrer | Use a real controlled company inbox to test OTP, personal-domain rejection, private impact summary, opaque and vanity Fast-Track Link creation, candidate packet, claim, approve/decline, conversation, and progress update. | OTP goes only to entered work email; private impact exposes only the current Referrer’s aggregates; either Fast-Track public resolution reveals only the exact company; unrelated employer cannot access any packet. |
