...
| **Private Fast-Track Link** | A verified employee wants to share a company-specific referral entry point. | Create one opaque, account-owned link for the exact verified company; resolve it publicly to company label only; assign a valid matching request only to the link owner. | Link open → valid private request rate. | Never expose employee identity, candidate rank, capacity, promise of review, or a bypass of the employer application process. |
...
| Referrer | Use a real controlled company inbox to test OTP, personal-domain rejection, Fast-Track Link creation, candidate packet, claim, approve/decline, conversation, and progress update. | OTP goes only to entered work email; Fast-Track public resolution reveals only the exact company; unrelated employer cannot access any packet. |
