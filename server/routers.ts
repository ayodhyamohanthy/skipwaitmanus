...
export const appRouter = router({
  ...
  referrals: router({ listMine: protectedProcedure.query(({ ctx }) => db.listReferralRequests(ctx.user.id)), create: protectedProcedure.input(z.object({ jobId: z.number().int().positive(), referrerId: z.number().int().positive(), personalPitch: z.string().min(20).max(600), attachmentIds: z.array(z.number().int().positive()).max(8).optional() })).mutation(({ ctx, input }) => db.createReferralRequest(ctx.user.id, input)), review: protectedProcedure.input(z.object({ requestId: z.number().int().positive(), decision: z.enum(["approved", "declined"]), message: z.string().max(1000).optional() })).mutation(({ ctx, input }) => db.reviewReferralRequest(ctx.user.id, input)), stats: protectedProcedure.query(({ ctx }) => db.getDashboardStats(ctx.user.id)) }),
