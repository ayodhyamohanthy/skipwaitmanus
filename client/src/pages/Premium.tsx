      ...
      <div className="mx-auto max-w-3xl"><div className="flex items-center"><Brand /></div>
        ...
        <section className="mt-6 grid gap-8 rounded-2xl border border-slate-200 bg-white p-8 shadow-sm lg:grid-cols-[1.1fr_.9fr]">
          <div>
            ...
            <div className="mt-7"><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-500">Popular packs</p><div className="mt-3 grid grid-cols-2 gap-2">{quickPacks.map(preset => { const presetTotal = selected.price * preset; const presetLabel = selected.currency === "INR" ? `₹${presetTotal.toLocaleString("en-IN")} INR` : `$${presetTotal.toLocaleString("en-US")} USD`; return <button type="button" key={preset} aria-pressed={quantity === preset} onClick={() => updateQuantity(preset)} className={`rounded-xl border p-3 text-left transition ${quantity === preset ? "border-blue-300 bg-blue-50" : "border-slate-200 hover:border-blue-200 hover:bg-slate-50"}`}><span className="block text-sm font-bold text-slate-900">{preset} tokens</span><span className="mt-0.5 block text-xs font-semibold text-[#0B57D0]">{presetLabel}</span></button>; })}</div></div>
            <div className="mt-5"><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-500">Or choose a custom quantity</p><div className="mt-3 inline-flex items-center rounded-xl border border-slate-200 bg-white shadow-sm"><button type="button" aria-label="Remove one token" onClick={() => updateQuantity(quantity - 1)} disabled={quantity <= 1} className="grid h-12 w-12 place-items-center text-xl font-semibold text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-35">−</button><label className="sr-only" htmlFor="token-quantity">Number of tokens to add</label><input id="token-quantity" aria-label="Number of tokens to add" type="number" min="1" max="1000" inputMode="numeric" value={quantity} onChange={event => updateQuantity(Number(event.target.value))} className="h-12 w-20 border-x border-slate-200 text-center text-base font-bold text-slate-950 outline-none focus:bg-blue-50" /><button type="button" aria-label="Add one token" onClick={() => updateQuantity(quantity + 1)} className="grid h-12 w-12 place-items-center text-xl font-semibold text-[#0B57D0] hover:bg-blue-50">+</button></div><p className="mt-2 text-xs text-slate-500">Add multiple tokens now; use them whenever you need a referral.</p></div>
          </div>
