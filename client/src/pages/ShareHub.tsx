...
export default function ShareHub() {
  ...
  const emailInviteHref = useMemo(() => {
    const recipient = email.trim();
    if (!link || !/^\S+@\S+\.\S+$/.test(recipient)) return "";
    return `mailto:${recipient}?subject=${encodeURIComponent("You found an opportunity. Help a friend do the same.")}&body=${encodeURIComponent(message)}`;
  }, [email, link, message]);
