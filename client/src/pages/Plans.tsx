...
const plans = { pro: { name: "Pro", monthlyCredits: 10, INR: "₹599/month", USD: "$7/month" }, max: { name: "Max", monthlyCredits: 30, INR: "₹1,299/month", USD: "$15/month" } } as const;
...
export default function Plans() {
  ...
  const price = useMemo(() => plans[selected][route], [route, selected]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      } catch { /* Keep comparison available if the secure account summary retries. */ }
    })();
    return () => { active = false; };
  ...
  const startCheckout = async () => {
    ...
    setError(""); setStatus("opening");
    try {
      const clerkToken = await getToken();
      const response = await fetch("/api/chargebee/subscription-checkout", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json", ...(clerkToken ? { Authorization: `Bearer ${clerkToken}` } : {}) }, body: JSON.stringify({ plan: selected, currency: route, billingCountry: routeDetails.billingCountry, role }) });
      const payload = await response.json().catch(() => ({}));
    ...
    } catch (checkoutError) { setError(checkoutError instanceof Error ? checkoutError.message : "Unable to open secure plan checkout"); setStatus("idle"); }
  };
  ...
  const scheduleCancellation = async () => {
    ...
    try {
      const clerkToken = await getToken();
      const response = await fetch("/api/chargebee/subscription-cancel", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json", ...(clerkToken ? { Authorization: `Bearer ${clerkToken}` } : {}) }, body: JSON.stringify({ role }) });
      const payload = await response.json().catch(() => ({}));
