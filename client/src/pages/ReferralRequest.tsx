...
export default function ReferralRequest() {
  ...
  if (submitted) {
    if (coveragePending) {
      ...
      return <Shell tokens={summary.totalAvailable} label="credits available"><section data-skipwait-coverage-invite="true" className="flex min-h-0 flex-1 flex-col"><div className="flex flex-1 flex-col items-center justify-center"><span className="grid h-16 w-16 place-items-center rounded-3xl bg-blue-50 text-[#0B57D0]"><UsersRound className="h-7 w-7" /></span><p className="mt-5 text-center text-sm font-semibold text-slate-800">Your request is queued for {companyDomain}.</p><p className="mt-1 text-center text-xs leading-5 text-slate-500">1 credit used · We’ll follow up as coverage grows.</p></div>{coverageInviteStatus && <p role="status" className="sr-only">{coverageInviteStatus}</p>}<footer className="shrink-0 pb-[max(0.75rem,env(safe-area-inset-bottom))]"><button type="button" aria-label={`Invite one employee at ${companyDomain}`} onClick={() => { void inviteEmployee(); }} className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#0B57D0] px-5 py-3.5 text-sm font-semibold text-white"><Share2 className="h-4 w-4" />Invite at {companyDomain}</button></footer></section></Shell>;
    }
    const used = Math.max(0, summary.monthlyAllowance - summary.monthlyCreditsRemaining);
    const exhausted = summary.totalAvailable === 0;
    const isFree = summary.plan === "free";
