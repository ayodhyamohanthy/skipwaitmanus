...
const INCLUDED_REQUEST_CREDITS = 3;
...
export default function ReferralRequest() {
  ...
  const [includedCreditsUsed, setIncludedCreditsUsed] = useState<number | null>(null);
  ...
  const send = async () => {
    ...
    try {
      ...
      const nextTokens = Number.isFinite(serverTokens) && serverTokens >= 0 ? serverTokens : Math.max(0, tokens - TOKEN_ACTION_COST);
      setTokens(nextTokens);
      setJobSeekerTokens(nextTokens);
      setIncludedCreditsUsed(tokens <= INCLUDED_REQUEST_CREDITS ? Math.min(INCLUDED_REQUEST_CREDITS, Math.max(0, INCLUDED_REQUEST_CREDITS - nextTokens)) : null);
  ...
  if (submitted) return <Shell tokens={tokens} label="credits left"><section className="flex min-h-0 flex-1 flex-col"><div className="flex flex-1 flex-col justify-center"><span className="grid h-12 w-12 place-items-center rounded-xl bg-emerald-50 text-emerald-700"><CheckCircle2 className="h-6 w-6" /></span><p className="mt-6 text-xs font-bold uppercase tracking-[.16em] text-[#0B57D0]">Private request created</p><h1 className="mt-3 text-[2.35rem] font-semibold leading-[.96] tracking-[-.06em]">Your request is with verified employees.</h1><p className="mt-4 text-sm leading-6 text-slate-600">Eligible employees at <strong>{companyDomain}</strong> were notified privately. Their identity stays hidden unless someone claims your request.</p><section aria-label="Credit progress" className="mt-5 rounded-xl border border-blue-100 bg-blue-50/70 p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold uppercase tracking-[.12em] text-[#0B57D0]">{includedCreditsUsed === null ? "Credit balance" : "Included credits"}</p><p className="text-sm font-bold text-slate-900">{includedCreditsUsed === null ? `${tokens} ready` : `${includedCreditsUsed} of ${INCLUDED_REQUEST_CREDITS} used`}</p></div>{includedCreditsUsed !== null && <><div role="progressbar" aria-label="Included credits used" aria-valuemin={0} aria-valuemax={INCLUDED_REQUEST_CREDITS} aria-valuenow={includedCreditsUsed} className="mt-3 h-2 overflow-hidden rounded-full bg-blue-100"><span className="block h-full rounded-full bg-[#0B57D0]" style={{ width: `${(includedCreditsUsed / INCLUDED_REQUEST_CREDITS) * 100}%` }} /></div><p className="mt-3 text-sm leading-5 text-slate-700">You used 1 included credit on this request. <strong>{tokens} left.</strong></p></>}<Link href="/premium?role=job_seeker" className="mt-3 inline-flex items-center gap-1 text-sm font-bold text-[#0B57D0]">Add 1, 5, or 10 credits <ArrowRight className="h-4 w-4" /></Link></section></div><footer className="shrink-0 border-t border-slate-200 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-4"><button type="button" onClick={() => setShowEmailDraft(true)} className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#0B57D0] px-5 py-3.5 text-sm font-semibold text-white">Prepare your referral email <ArrowRight className="h-4 w-4" /></button><Link href="/requests" className="mt-3 block text-center text-sm font-semibold text-slate-600">View my request</Link></footer></section></Shell>;
