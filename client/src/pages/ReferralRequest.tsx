...
export default function ReferralRequest() {
  ...
  const handleSend = async () => { if (!attachmentCount) { setError("Add your resume before sending this request."); return; } if (!isSignedIn) { sessionStorage.setItem(pendingResumeSubmissionKey, "true"); await savePendingResumeFiles(pendingFiles).catch(() => undefined); openSignIn(); return; } void send(); };
