...
Implementation verification on the Chargebee Test site confirmed active one-month item prices for Pro (₹599 / $7) and Max (₹1,299 / $15). The dashboard owner session is available and exposes Settings navigation, while the attempted generic v2 `/webhooks` listing endpoint returned 404; webhook event coverage must therefore be inspected in the dashboard UI rather than assumed from the catalog API.
